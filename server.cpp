    ...
    if (activity > 0 && FD_ISSET(sockfd, &readfds)) {
        ...
        if (bytesReceived > 0) {
            ...
            // Check if the response starts with "SERVERS,"
                ...
                std::string serversList = unframedResponse.substr(8); // Adjusted to account for the comma
                ...
                for (const auto& serverInfo : servers) {
                    ...
                    if (fields.size() == 3) {
                        ...
                        // Add to serverList if not already present
                        bool found = false;
                        for (const auto& existingServer : serverList) {
                            if (existingServer.groupID == groupID) {
                                found = true;
                                break;
                            }
                        }
                ...
                return ""; // or return some appropriate value
            } else {
        ...
        for (const auto& server : connectedServers) {
            ...
            if (result < 0) {
                ...
                if (bytesReceived > 0) {
                    std::string response(buffer, bytesReceived);
                    std::string unframedResponse = unframeMessage(response);
                    logMessage("DEBUG", "Unframed response from server: " + unframedResponse);
